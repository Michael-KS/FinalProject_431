...
std::string YourProposalFunction(
    ...
    */    
    ...
    // optimize for sim_IPC and clock cycle time (performance)
    // max width and out-of-order 
    if(optimizeforED2P == 1){
        configuration[0] = 3; //max width
        configuration[1] = 1; //max fetchspeed
        configuration[2] = 1; //dynamic
        configuration[3] = 5; //max RUU size
        configuration[4] = 3; //max LSQ size
